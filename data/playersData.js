function getPlayerStats(player, allPlayers) {
  ...
  const outsideScoring =
    normalize(+player["3P%"], threePts, threePtsAverage) * 100;
  ...
  const insideScoring = normalize(+player["2P%"], twoPts, twoPtsAverage) * 100;
  ...
  const freeThrow =
    normalize(+player["FT%"], freeThrows, freeThrowAverage) * 100;
  ...
  const playmaking = normalize(+player.AST, assists, assistsAverage) * 100;
  ...
  const rebounding =
    normalize(playerRebounding, rebounds, reboundsAverage) * 100;
  ...
  const defending =
    normalize(playerDefensiveStates, defenseStats, defenseStatsAverage) * 100;
  ...
  function normalize(val, allVal, extraMax = 0) {
    ...
    let max = Math.max(...allVal) - extraMax / 2;
